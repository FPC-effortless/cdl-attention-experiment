...
def _execute_with_gates(model,ep,x,gates):
    ...
    for node in ep.nodes[:ep.active_count]:
        ...
        if node.op.value=="NOT": vals[node.index]=1-alpha[0]*args[0]
            ...
            a,b=alpha[0]*args[0],alpha[1]*args[1]
...
def gate0_viability(model,episodes):
    device=next(model.parameters()).device; x=_batch_x(episodes,device); y,g,_=model(episodes,x); nz=g[g!=0]
    mean_g=float(nz.mean()); std_g=float(nz.std()); trace=model.last_node_values; variances=[]
...
def gate1_causality(model,episode):
    device=next(model.parameters()).device; x=torch.tensor([episode.input_values],dtype=torch.float32,device=device); y,g,_=model([episode],x); k=int(torch.argmax(g[0]).item()); edited=g[0].detach().clone(); edited[k]=0; cf=_execute_with_gates(model,episode,x[0],edited); delta=abs(float(y.item())-float(cf.item()))
...
def gate3_structural_sensitivity(model,ep_a,ep_b):
    device=next(model.parameters()).device; _,g,_=model([ep_a,ep_b],_batch_x([ep_a,ep_b],device)); n=min(len(ep_a.candidate_edges),len(ep_b.candidate_edges)); diff=float((g[0,:n]-g[1,:n]).abs().mean())
...
def gate5_compositional_ood(model,episode,source_op,target_op):
    device=next(model.parameters()).device; _,g,_=model([episode],_batch_x([episode],device)); vals=[]
    for k,e in enumerate(episode.candidate_edges):
        if episode.nodes[e.src].op.value==source_op and episode.nodes[e.dst].op.value==target_op: vals.append((float(g[0,k]),(e.src,e.dst,e.port) in episode.true_edge_set))
    pos=[v for v,t in vals if t]; neg=[v for v,t in vals if not t]; sep=(sum(pos)/len(pos)-sum(neg)/len(neg)) if pos and neg else 0
    return {"pass":bool(pos and sep>0),"role_pair":f"{source_op}->{target_op}","positive_mean":sum(pos)/len(pos) if pos else None,"negative_mean":sum(neg)/len(neg) if neg else None,"separation":sep}
...
def gate6_integrity(model,episodes):
    ...
    for ep in episodes: ok=ok and all(e.src<e.dst for e in ep.candidate_edges) and all(len(ep.existence_mask)==len(ep.nodes) and e.src<ep.active_count and e.dst<ep.active_count for e in ep.true_edges)
