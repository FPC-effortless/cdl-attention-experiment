...
HARNESS_VERSION = "casm-x20t-counterfactual-credit-v0-2026-09-05"
LR_MAX = 2e-3
LR_MIN = 2e-4
WEIGHT_DECAY = 1e-4
GRAD_CLIP_NORM = 1.0
...
def train_step(model, optimizer, batch, *, mode: str, lr: float):
    ...
    parts = credit_loss_components(model, batch, mode=mode)
...
def main():
    ...
    p.add_argument("--eval-seed", type=int, default=20261281)
    p.add_argument("--eval-n", type=int, default=256)
    p.add_argument("--eval-batch-size", type=int, default=64)
    p.add_argument("--output", type=Path, default=Path("casm-x20t-output/results.json"))
    args = p.parse_args()

    if args.steps != PREREGISTERED_STEPS or args.batch_size != 128 or args.train_depth != 12 or args.eval_n != 256:
        raise ValueError("CASM-X20T frozen run contract mismatch")

    torch.manual_seed(args.seed)
    random.seed(args.seed)
    torch.set_num_threads(max(1, min(4, torch.get_num_threads())))

    models = cloned_x20t_models(d_model=96)
    optimizers = {
        mode: torch.optim.AdamW(model.parameters(), lr=LR_MAX, weight_decay=WEIGHT_DECAY)
        for mode, model in models.items()
    }
    learned_counts = {m: models[m].parameter_count() for m in X20T_LEARNED_MODES}
    learned_trainable = {m: models[m].trainable_parameter_count() for m in X20T_LEARNED_MODES}
    ...
    minimum_task_loss = {m: float("inf") for m in X20T_MODES}
    final_task_loss = {m: float("nan") for m in X20T_MODES}
    minimum_soft_answer_loss = {m: float("inf") for m in X20T_MODES}
    minimum_hard_answer_loss = {m: float("inf") for m in X20T_MODES}
    maximum_post4000_grad_norm = {m: 0.0 for m in X20T_MODES}
    history = []
    ...
    report = {
        ...
        "question": "does same-target counterfactual soft execution provide usable credit for robust discrete state existence?",
        "seed": args.seed,
        ...
        "parameters": {m: models[m].parameter_count() for m in X20T_MODES},
        "trainable_parameters": {m: models[m].trainable_parameter_count() for m in X20T_MODES},
        ...
        "objective_contract": {
            HARD_ONLY_MODE: "A_hard + 0.05*S_hard",
            SOFT_X20_MODE: "A_soft + 0.05*S_soft",
            SOFT_CREDIT_MODE: "A_soft + 0.05*S_hard",
            DUAL_CREDIT_MODE: "0.5*A_hard + 0.5*A_soft + 0.05*S_hard",
            SOFT_CREDIT_BLIND_MODE: "A_soft + 0.05*S_hard",
            "storage_lambda": STORAGE_LAMBDA,
            "same_final_answer_target_for_hard_and_soft": True,
            "counterfactual_soft_path_adds_no_labels": True,
        },
        "constructor_contract": {
            ...
            "graph_conditioned_modes": list(X20T_GRAPH_MODES),
            "credit_graph_modes": list(X20T_CREDIT_GRAPH_MODES),
            "structure_blind_ablation": SOFT_CREDIT_BLIND_MODE,
        },
