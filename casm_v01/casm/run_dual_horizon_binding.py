...
from .dual_horizon_binding import (
    DIAGNOSTIC_MODE,
    DUAL_ETA,
    LEARNED_X17_MODES,
    LONG_DUAL_ROUNDS,
...
HARNESS_VERSION = "casm-x17-dual-horizon-v0-2026-09-04"
EVAL_MODES = X17_TRAIN_MODES + (DIAGNOSTIC_MODE,)
...
def train_step(model, optimizer, batch, *, applied_lr: float):
    ...
    binding, prices = model.soft_binding_and_prices(batch.initial.shape[1])
    if not torch.isfinite(binding).all() or (binding < 0.0).any():
        raise RuntimeError("invalid primal binding during training")
    if not torch.isfinite(prices).all() or (prices < 0.0).any():
        raise RuntimeError("invalid dual prices during training")
    if not torch.allclose(binding.sum(dim=1), torch.ones(binding.shape[0], device=binding.device), atol=1e-5, rtol=0.0):
        raise RuntimeError("primal binding rows are not normalized")
    parts["total_loss"].backward()
    grad_norm = torch.nn.utils.clip_grad_norm_(model.parameters(), GRAD_CLIP_NORM)
    optimizer.step()
    values["grad_norm"] = float(grad_norm)
    values["lr"] = float(applied_lr)
    values["max_dual_price"] = float(prices.max().detach())
    return values
...
def main():
    ...
    p.add_argument("--eval-seed", type=int, default=20261201)
    p.add_argument("--eval-n", type=int, default=256)
    p.add_argument("--eval-batch-size", type=int, default=64)
    p.add_argument("--output", type=Path, default=Path("casm-x17-output/results.json"))
    args = p.parse_args()

    if args.steps != PREREGISTERED_STEPS:
        raise ValueError("CASM-X17 preregisters exactly 10000 optimization steps")
    if args.batch_size != 128 or args.train_depth != 8:
        raise ValueError("CASM-X17 preregisters batch size 128 and train depth 8")
    if args.weight_decay != WEIGHT_DECAY or args.eval_n != 256:
        raise ValueError("CASM-X17 frozen weight decay/eval_n mismatch")
    ...
    assert parameters["dual_short_8"] == parameters["dual_long_64"]
    assert trainable_parameters["dual_short_8"] == trainable_parameters["dual_long_64"]

    initial_bindings_train_cardinalities_only = {
        mode: {str(n): models[mode].binding_stats(n) for n in TRAIN_CARDINALITIES}
        for mode in LEARNED_X17_MODES
    }

    minimum_answer_loss = {mode: float("inf") for mode in X17_TRAIN_MODES}
    minimum_total_loss = {mode: float("inf") for mode in X17_TRAIN_MODES}
    final_answer_loss = {mode: float("nan") for mode in X17_TRAIN_MODES}
    final_total_loss = {mode: float("nan") for mode in X17_TRAIN_MODES}
    maximum_post4000_grad_norm = {mode: 0.0 for mode in X17_TRAIN_MODES}
    maximum_training_dual_price = {mode: 0.0 for mode in X17_TRAIN_MODES}
    ...
    # The diagnostic view is created only after all optimizer steps are complete.
    diagnostic = short_eval64_view(models["dual_short_8"])
    eval_models = dict(models)
    eval_models[DIAGNOSTIC_MODE] = diagnostic

    # Unseen cardinalities are first touched here, after optimization is complete.
    final_bindings = {
        mode: {str(n): eval_models[mode].binding_stats(n) for n in range(MIN_CARDINALITY, MAX_CARDINALITY + 1)}
        for mode in ("dual_short_8", "dual_long_64", DIAGNOSTIC_MODE)
    }
    ...
    for num_registers in range(MIN_CARDINALITY, MAX_CARDINALITY + 1):
        ...
        for suite_index, (suite, split, depth) in enumerate(suites):
            ...
            for mode in EVAL_MODES:
                model = eval_models[mode]
                evaluation[nkey][suite][mode] = evaluate(
                    model, num_registers=num_registers, depth=depth, split=split,
    ...
    report = {
        ...
        "question": "is X16 unseen failure explained by an 8-round dual convergence horizon?",
        "seed": args.seed,
        ...
        "dual_contract": {
            "short_rounds": SHORT_DUAL_ROUNDS,
            "long_rounds": LONG_DUAL_ROUNDS,
            "eta": DUAL_ETA,
            "initial_prices": "zeros(8)",
            "primal_formula": "P=softmax(base_logits-lambda)",
            "occupancy_formula": "c=sum_rows(P)",
            "dual_formula": "lambda=relu(lambda + (c-1))",
        ...
        "diagnostic_contract": {
            ...
            "source_model": "dual_short_8",
            "inference_rounds": LONG_DUAL_ROUNDS,
            "optimizer_steps_after_copy": 0,
            "created_after_training": True,
        },
