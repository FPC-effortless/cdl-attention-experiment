...
HARNESS_VERSION = "casm-x6-learned-binding-v0-2026-08-31"
...
def main():
    ...
    p.add_argument("--sinkhorn-iterations", type=int, default=12)
    p.add_argument("--seed", type=int, default=20260911)
    p.add_argument("--eval-seed", type=int, default=20260991)
    p.add_argument("--eval-n", type=int, default=384)
    p.add_argument("--eval-batch-size", type=int, default=64)
    p.add_argument("--output", type=Path, default=Path("casm-x6-output/results.json"))
    args = p.parse_args()

    if args.train_depth != 8:
        raise ValueError("CASM-X6 v0 preregisters fixed training depth 8")
    if args.steps != 6000:
        raise ValueError("CASM-X6 v0 preregisters exactly 6000 optimization steps")
