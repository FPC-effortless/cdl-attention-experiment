...
from .coordinated_binding import LEARNED_MODES, X10_MODES, cloned_x10_models
...
HARNESS_VERSION = "casm-x10-coordinated-binding-v0-2026-08-31"
...
def main():
    ...
    p.add_argument("--seed", type=int, default=20261051)
    p.add_argument("--eval-seed", type=int, default=20261131)
    p.add_argument("--eval-n", type=int, default=256)
    p.add_argument("--eval-batch-size", type=int, default=64)
    p.add_argument("--output", type=Path, default=Path("casm-x10-output/results.json"))
    args = p.parse_args()

    if args.steps != PREREGISTERED_STEPS:
        raise ValueError("CASM-X10 preregisters exactly 10000 optimization steps")
    if args.batch_size != 128:
        raise ValueError("CASM-X10 preregisters batch size 128")
    if args.train_depth != 8:
        raise ValueError("CASM-X10 preregisters train depth 8")
    if args.weight_decay != WEIGHT_DECAY:
        raise ValueError("CASM-X10 preregisters weight decay 1e-4")
    if args.eval_n != 256:
        raise ValueError("CASM-X10 preregisters eval_n=256 per cardinality/suite")

    torch.manual_seed(args.seed)
    random.seed(args.seed)
    torch.set_num_threads(max(1, min(4, torch.get_num_threads())))

    models = cloned_x10_models(d_model=96)
    optimizers = {
    ...
    assert parameters["relational_independent"] == parameters["relational_coordinated"]
    assert trainable_parameters["relational_independent"] == trainable_parameters["relational_coordinated"]

    relational_left = models["relational_independent"].binding_generator.state_dict()
    relational_right = models["relational_coordinated"].binding_generator.state_dict()
    assert relational_left.keys() == relational_right.keys()
    assert all(torch.equal(relational_left[k], relational_right[k]) for k in relational_left)
    ...
    minimum_training_loss = {mode: float("inf") for mode in X10_MODES}
    final_training_loss = {mode: float("nan") for mode in X10_MODES}
    maximum_post4000_grad_norm = {mode: 0.0 for mode in X10_MODES}
    ...
    for step in range(1, args.steps + 1):
        ...
        for mode in X10_MODES:
            ...
            loss_value = row[mode]["loss"]
            grad_value = row[mode]["grad_norm"]
            minimum_training_loss[mode] = min(minimum_training_loss[mode], loss_value)
            final_training_loss[mode] = loss_value
            if step > 4000:
    ...
    report = {
        ...
        "question": "does relational slot factorization and/or cross-variable coordination enable unseen-cardinality binding generalization on the validated local executor?",
        "seed": args.seed,
        ...
        "supervision_contract": {
            ...
            "injective_constraint": False,
            "matching_or_collision_repair": False,
