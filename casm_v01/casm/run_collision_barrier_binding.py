...
from .collision_barrier_binding import (
    BARRIER_EPSILON,
    BARRIER_MODES,
    LEARNED_X13_MODES,
    QUADRATIC_MODES,
...
HARNESS_VERSION = "casm-x13-collision-barrier-v0-2026-09-01"
...
def train_step(model, optimizer, batch, *, applied_lr: float):
    ...
    for key in ("spread_penalty", "capacity_penalty", "barrier_penalty", "total_loss"):
...
def main():
    ...
    p.add_argument("--seed", type=int, default=20261081)
    p.add_argument("--eval-seed", type=int, default=20261161)
    p.add_argument("--eval-n", type=int, default=256)
    p.add_argument("--eval-batch-size", type=int, default=64)
    p.add_argument("--output", type=Path, default=Path("casm-x13-output/results.json"))
    args = p.parse_args()

    if args.steps != PREREGISTERED_STEPS:
        raise ValueError("CASM-X13 preregisters exactly 10000 optimization steps")
    if args.batch_size != 128 or args.train_depth != 8:
        raise ValueError("CASM-X13 preregisters batch size 128 and train depth 8")
    if args.weight_decay != WEIGHT_DECAY or args.eval_n != 256:
        raise ValueError("CASM-X13 frozen weight decay/eval_n mismatch")

    torch.manual_seed(args.seed)
    random.seed(args.seed)
    torch.set_num_threads(max(1, min(4, torch.get_num_threads())))

    models = cloned_x13_models(d_model=96)
    optimizers = {
    ...
    minimum_answer_loss = {mode: float("inf") for mode in X13_MODES}
    minimum_total_loss = {mode: float("inf") for mode in X13_MODES}
    final_answer_loss = {mode: float("nan") for mode in X13_MODES}
    final_total_loss = {mode: float("nan") for mode in X13_MODES}
    maximum_post4000_grad_norm = {mode: 0.0 for mode in X13_MODES}
    ...
    report = {
        ...
        "question": "does a saturation-resistant logarithmic collision barrier repair X12 quadratic scarcity allocation?",
        "seed": args.seed,
        ...
        "barrier_contract": {
            ...
            "spread_lambda": 1.0,
            "barrier_lambda": 1.0,
            "quadratic_modes": list(QUADRATIC_MODES),
            "barrier_modes": list(BARRIER_MODES),
            "barrier_formula": "mean_ij -log(1 - (1-epsilon) * dot(binding_i,binding_j))",
            "quadratic_formula": "sum_s relu(column_occupancy_s - 1)^2 / n",
