...
from .process_supervision import ProcessHead, process_alignment_loss
...
def train_one(
    ...
    init_head: Dict[str, torch.Tensor],
) -> Tuple[CASMRecurrent, ProcessHead, List[Dict[str, float]], float]:
    model = build_core(cfg)
    model.load_state_dict(init_core)
    head = ProcessHead(cfg.d_model, args.process_code_dim)
    ...
    for step in range(1, args.steps + 1):
        ...
        toks, pex, answer_mask, anchors = make_process_batch(
            args.batch_size,
            args.seq_len,
        ...
        process_loss = out["loss"].new_zeros(())
        process_cos = out["loss"].new_zeros(())
        process_weight = 0.0

        if kind == "process-3step" and step >= args.process_start:
            process_loss, process_cos = process_alignment_loss(
                model,
                head,
                toks,
                anchors,
                [x.traces for x in pex],
            )
            progress = (step - args.process_start + 1) / max(1, args.process_warmup)
            process_weight = args.process_weight * min(1.0, progress)

        total = out["loss"] + process_weight * process_loss
        ...
        if step == 1 or step % args.log_every == 0 or step == args.steps:
            row = {
                ...
                "process_cosine": float(process_cos),
    ...
    return model, head, history, time.perf_counter() - started


def main() -> None:
    p = argparse.ArgumentParser()
    p.add_argument("--out", default="casm-p-output")
    ...
    p.add_argument("--process-start", type=int, default=200)
    p.add_argument("--process-warmup", type=int, default=400)
    p.add_argument("--process-weight", type=float, default=0.15)
    p.add_argument("--process-code-dim", type=int, default=64)
    p.add_argument("--tiny", action="store_true")
    p.add_argument("--models", default="final-only-3step,process-3step")
    args = p.parse_args()
    ...
    probe_head = ProcessHead(cfg.d_model, args.process_code_dim)
    init_head = {k: v.detach().clone() for k, v in probe_head.state_dict().items()}

    out_dir = Path(args.out)
    out_dir.mkdir(parents=True, exist_ok=True)
    summary = []
    for kind in [x.strip() for x in args.models.split(",") if x.strip()]:
        if kind not in {"final-only-3step", "process-3step"}:
            raise ValueError(kind)
        model, head, history, elapsed = train_one(kind, args, cfg, init_core, init_head)
        easy = evaluate_process(model, args.seed + 9_000_000, args.eval_batches, args.batch_size, args.seq_len, False)
        hard = evaluate_process(model, args.seed + 19_000_000, args.eval_batches, args.batch_size, args.seq_len, True)
        torch.save(
            {
        ...
        torch.save(head.state_dict(), out_dir / f"{kind}-process-head.pt")
        (out_dir / f"{kind}-history.json").write_text(json.dumps(history, indent=2))
        summary.append(
            {
                "model": kind,
                "deployed_parameters": model.parameter_count(),
                "training_process_head_parameters": sum(p.numel() for p in head.parameters()),
                "seconds": elapsed,
