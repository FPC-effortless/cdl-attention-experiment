...
from .collision_barrier_binding import BARRIER_EPSILON
from .occupancy_state_binding import (
    ITERATIVE_MODES,
    LEARNED_X14_MODES,
    REFINEMENT_ROUNDS,
    X14_MODES,
...
HARNESS_VERSION = "casm-x14-occupancy-state-binding-v0-2026-09-03"
...
def main():
    ...
    parser.add_argument("--steps", type=int, default=PREREGISTERED_STEPS)
    parser.add_argument("--batch-size", type=int, default=128)
    parser.add_argument("--train-depth", type=int, default=8)
    parser.add_argument("--weight-decay", type=float, default=WEIGHT_DECAY)
    parser.add_argument("--seed", type=int, default=20261091)
    parser.add_argument("--eval-seed", type=int, default=20261171)
    parser.add_argument("--eval-n", type=int, default=256)
    parser.add_argument("--eval-batch-size", type=int, default=64)
    parser.add_argument("--output", type=Path, default=Path("casm-x14-output/results.json"))
    args = parser.parse_args()

    if args.steps != PREREGISTERED_STEPS:
        raise ValueError("CASM-X14 preregisters exactly 10000 optimization steps")
    if args.batch_size != 128 or args.train_depth != 8:
        raise ValueError("CASM-X14 preregisters batch size 128 and train depth 8")
    if args.weight_decay != WEIGHT_DECAY or args.eval_n != 256:
        raise ValueError("CASM-X14 frozen weight decay/eval_n mismatch")

    torch.manual_seed(args.seed)
    random.seed(args.seed)
    torch.set_num_threads(max(1, min(4, torch.get_num_threads())))

    models = cloned_x14_models(d_model=96)
    optimizers = {
    ...
    assert parameters["iterative_no_occupancy"] == parameters["iterative_occupancy"]
    assert trainable_parameters["iterative_no_occupancy"] == trainable_parameters["iterative_occupancy"]

    # Do not touch n=5,6 before training; X14 freezes them as fully unseen to every train-time loss
    # and pre-training binding diagnostic.
    initial_bindings = {
        mode: {str(n): models[mode].binding_stats(n) for n in TRAIN_CARDINALITIES}
        for mode in LEARNED_X14_MODES
    }

    minimum_answer_loss = {mode: float("inf") for mode in X14_MODES}
    minimum_total_loss = {mode: float("inf") for mode in X14_MODES}
    final_answer_loss = {mode: float("nan") for mode in X14_MODES}
    final_total_loss = {mode: float("nan") for mode in X14_MODES}
    maximum_post4000_grad_norm = {mode: 0.0 for mode in X14_MODES}
    cardinality_counts = {str(n): 0 for n in TRAIN_CARDINALITIES}
    history = []

    for step in range(1, args.steps + 1):
        num_registers = training_cardinality_for_step(step)
        if num_registers not in TRAIN_CARDINALITIES:
            raise RuntimeError(f"unexpected train cardinality {num_registers}")
    ...
    report = {
        ...
        "question": "does explicit generated soft slot occupancy enable X13 barrier allocation to extrapolate to unseen cardinality?",
        "seed": args.seed,
        ...
        "occupancy_contract": {
            ...
            "other_occupancy_formula": "sum_j!=i current_binding[j,slot]",
            "no_occupancy_ablation": "same refiner and rounds with other occupancy clamped to zero",
            "row_permutation_equivariant": True,
            "slot_permutation_equivariant": True,
            "hard_masking": False,
            "hard_injectivity": False,
            "matching": False,
            "sinkhorn": False,
            "sorting_assignment": False,
        ...
        "barrier_contract": {
            ...
            "spread_lambda": 1.0,
            "barrier_lambda": 1.0,
            "barrier_formula": "mean_ij -log(1 - (1-epsilon) * dot(binding_i,binding_j))",
        },
        "supervision_contract": {
            ...
            "unseen_train_loss_calls": 0,
            "hard_binding_evaluation": "independent row argmax without collision repair",
        },
        "initial_bindings_train_cardinalities_only": initial_bindings,
