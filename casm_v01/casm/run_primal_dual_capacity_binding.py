...
from .primal_dual_capacity_binding import (
    DUAL_ETA,
    DUAL_ROUNDS,
    LEARNED_X16_MODES,
    X16_MODES,
...
HARNESS_VERSION = "casm-x16-primal-dual-capacity-v0-2026-09-04"
...
def main():
    ...
    p.add_argument("--seed", type=int, default=20261111)
    p.add_argument("--eval-seed", type=int, default=20261191)
    p.add_argument("--eval-n", type=int, default=256)
    p.add_argument("--eval-batch-size", type=int, default=64)
    p.add_argument("--output", type=Path, default=Path("casm-x16-output/results.json"))
    args = p.parse_args()

    if args.steps != PREREGISTERED_STEPS:
        raise ValueError("CASM-X16 preregisters exactly 10000 optimization steps")
    if args.batch_size != 128 or args.train_depth != 8:
        raise ValueError("CASM-X16 preregisters batch size 128 and train depth 8")
    if args.weight_decay != WEIGHT_DECAY or args.eval_n != 256:
        raise ValueError("CASM-X16 frozen weight decay/eval_n mismatch")

    torch.manual_seed(args.seed)
    random.seed(args.seed)
    torch.set_num_threads(max(1, min(4, torch.get_num_threads())))

    models = cloned_x16_models(d_model=96)
    optimizers = {
    ...
    assert parameters["dual_neutral"] == parameters["dual_priced"]
    assert trainable_parameters["dual_neutral"] == trainable_parameters["dual_priced"]
    ...
    initial_bindings_train_cardinalities_only = {
        ...
        for mode in LEARNED_X16_MODES
    }

    minimum_answer_loss = {mode: float("inf") for mode in X16_MODES}
    minimum_total_loss = {mode: float("inf") for mode in X16_MODES}
    final_answer_loss = {mode: float("nan") for mode in X16_MODES}
    final_total_loss = {mode: float("nan") for mode in X16_MODES}
    maximum_post4000_grad_norm = {mode: 0.0 for mode in X16_MODES}
    maximum_training_dual_price = {mode: 0.0 for mode in X16_MODES}
    ...
    report = {
        ...
        "question": "does persistent projected dual-price state enable robust cardinality-generalizing allocation?",
        "seed": args.seed,
        ...
        "dual_contract": {
            ...
            "dual_formula": "lambda=relu(lambda + eta*(c-1))",
