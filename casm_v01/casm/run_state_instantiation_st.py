...
from .state_instantiation_model import STORAGE_LAMBDA
...
HARNESS_VERSION = "casm-x20r-straight-through-instantiation-v0-2026-09-04"
...
def main():
    ...
    p.add_argument("--seed", type=int, default=20261181)
    p.add_argument("--eval-seed", type=int, default=20261261)
    p.add_argument("--eval-n", type=int, default=256)
    p.add_argument("--eval-batch-size", type=int, default=64)
    p.add_argument("--output", type=Path, default=Path("casm-x20r-output/results.json"))
    args = p.parse_args()

    if args.steps != PREREGISTERED_STEPS or args.batch_size != 128 or args.train_depth != 12 or args.eval_n != 256:
        raise ValueError("CASM-X20R frozen run contract mismatch")

    torch.manual_seed(args.seed)
    random.seed(args.seed)
    torch.set_num_threads(max(1, min(4, torch.get_num_threads())))

    models = cloned_x20r_models(d_model=96)
    optimizers = {
        mode: torch.optim.AdamW(model.parameters(), lr=LR_MAX, weight_decay=WEIGHT_DECAY)
        for mode, model in models.items()
    }
    learned_counts = {m: models[m].parameter_count() for m in X20R_LEARNED_MODES}
    assert len(set(learned_counts.values())) == 1

    initial_gate_stats_train_only = {}
    for mode in X20R_LEARNED_MODES:
    ...
    minimum_answer_loss = {m: float("inf") for m in X20R_MODES}
    final_answer_loss = {m: float("nan") for m in X20R_MODES}
    maximum_post4000_grad_norm = {m: 0.0 for m in X20R_MODES}
    history = []
    ...
    report = {
        ...
        "question": "does discrete straight-through training repair X20 fractional state occupancy?",
        "seed": args.seed,
        ...
        "parameters": {m: models[m].parameter_count() for m in X20R_MODES},
        "trainable_parameters": {m: models[m].trainable_parameter_count() for m in X20R_MODES},
        ...
        "constructor_contract": {
            ...
            "soft_replication": SOFT_MODE,
            "straight_through_graph": ST_GRAPH_MODE,
            "straight_through_blind": ST_BLIND_MODE,
            "straight_through_forward_binary": True,
            "straight_through_storage_uses_forward_hard_fraction": True,
