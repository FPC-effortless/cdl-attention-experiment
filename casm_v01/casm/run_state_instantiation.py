...
from .state_instantiation_model import LEARNED_MODES, STORAGE_LAMBDA, X20_MODES, cloned_x20_models
from .variable_cardinality_binding import EMPTY_VALUE

HARNESS_VERSION = "casm-x20-state-instantiation-v0-2026-09-04"
...
def train_step(model, optimizer, batch, *, lr: float):
...
def _capability_metrics(pred: torch.Tensor, target: torch.Tensor, live_mask: torch.Tensor) -> dict[str, float]:
    # pred/target: B,T,8 integer value states.
...
def main():
    ...
    p.add_argument("--seed", type=int, default=20261171)
    p.add_argument("--eval-seed", type=int, default=20261251)
    p.add_argument("--eval-n", type=int, default=256)
    p.add_argument("--eval-batch-size", type=int, default=64)
    p.add_argument("--output", type=Path, default=Path("casm-x20-output/results.json"))
    args = p.parse_args()

    if args.steps != PREREGISTERED_STEPS or args.batch_size != 128 or args.train_depth != 12 or args.eval_n != 256:
        raise ValueError("CASM-X20 frozen run contract mismatch")

    torch.manual_seed(args.seed)
    random.seed(args.seed)
    torch.set_num_threads(max(1, min(4, torch.get_num_threads())))

    models = cloned_x20_models(d_model=96)
    optimizers = {
        mode: torch.optim.AdamW(model.parameters(), lr=LR_MAX, weight_decay=WEIGHT_DECAY)
        for mode, model in models.items()
    }
    assert models["learned_instantiation"].parameter_count() == models["structure_blind_gate"].parameter_count()

    # Train-time diagnostics are restricted to n=2,3,4.
    ...
    for mode in LEARNED_MODES:
        ...
        for nlive in TRAIN_LIVE_CARDINALITIES:
            ...
            initial_gate_stats_train_only[mode][str(nlive)] = _selection_metrics(models[mode].gates(b), b.live_mask)

    cardinality_counts = {str(n): 0 for n in TRAIN_LIVE_CARDINALITIES}
    minimum_answer_loss = {m: float("inf") for m in X20_MODES}
    final_answer_loss = {m: float("nan") for m in X20_MODES}
    maximum_post4000_grad_norm = {m: 0.0 for m in X20_MODES}
    history = []
    ...
    for step in range(1, args.steps + 1):
        ...
        for mode in X20_MODES:
            models[mode].train()
            row[mode] = train_step(models[mode], optimizers[mode], batch, lr=lr)
        ...
        if step == 1 or step % 500 == 0 or step == args.steps:
            if step % 1000 == 0 or step in (1, args.steps):
                row["gate_diagnostics"] = {
                    mode: _selection_metrics(models[mode].gates(batch), batch.live_mask)
                    for mode in LEARNED_MODES
                }
            history.append(row)
    ...
    # n=5/6 are first generated/evaluated only after all optimizer steps finish.
    suites = [
        ("iid_depth_12", "iid", 12),
        ...
        for si, (suite, split, depth) in enumerate(suites):
            ...
            for mode in X20_MODES:
                evaluation[str(nlive)][suite][mode] = evaluate_mode(
                    models[mode], live_cardinality=nlive, depth=depth, split=split,
    ...
    report = {
        ...
        "question": "can answer-only learning plus memory cost infer which observed candidate states should exist?",
        "seed": args.seed,
        ...
        "parameters": {m: models[m].parameter_count() for m in X20_MODES},
        "trainable_parameters": {m: models[m].trainable_parameter_count() for m in X20_MODES},
        ...
        "optimizer_contract": {
            ...
            "lr_max": LR_MAX, "lr_min": LR_MIN, "steps": PREREGISTERED_STEPS,
        },
        "constructor_contract": {
            ...
            "structure_blind_ablation": True,
            "fixed_candidate_count": NUM_CANDIDATES,
