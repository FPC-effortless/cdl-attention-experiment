...
from .detached_dual_binding import LEARNED_X18R_MODES, X18R_MODES, cloned_x18r_models
from .primal_dual_capacity_binding import DUAL_ETA, DUAL_ROUNDS
...
from .variable_contextual_data import TRAIN_CARDINALITIES, make_variable_contextual_batch, training_cardinality_for_step

HARNESS_VERSION = "casm-x18r-detached-dual-gradients-v0-2026-09-04"
EVAL_CARDINALITIES = tuple(TRAIN_CARDINALITIES)
...
def main():
    ...
    p.add_argument("--seed", type=int, default=20261141)
    p.add_argument("--eval-seed", type=int, default=20261221)
    p.add_argument("--eval-n", type=int, default=256)
    p.add_argument("--eval-batch-size", type=int, default=64)
    p.add_argument("--output", type=Path, default=Path("casm-x18r-output/results.json"))
    args = p.parse_args()

    if args.steps != PREREGISTERED_STEPS:
        raise ValueError("CASM-X18R preregisters exactly 10000 optimization steps")
    if args.batch_size != 128 or args.train_depth != 8:
        raise ValueError("CASM-X18R preregisters batch size 128 and train depth 8")
    if args.weight_decay != WEIGHT_DECAY or args.eval_n != 256:
        raise ValueError("CASM-X18R frozen weight decay/eval_n mismatch")
    if EVAL_CARDINALITIES != (2, 3, 4):
        raise RuntimeError("CASM-X18R is seen-only by contract")

    torch.manual_seed(args.seed)
    random.seed(args.seed)
    torch.set_num_threads(max(1, min(4, torch.get_num_threads())))

    models = cloned_x18r_models(d_model=96)
    optimizers = {
    ...
    assert parameters["dual_fullgrad"] == parameters["dual_detached_prices"]
    assert trainable_parameters["dual_fullgrad"] == trainable_parameters["dual_detached_prices"]

    initial_bindings = {
        mode: {str(n): models[mode].binding_stats(n) for n in EVAL_CARDINALITIES}
        for mode in LEARNED_X18R_MODES
    }

    minimum_answer_loss = {mode: float("inf") for mode in X18R_MODES}
    minimum_total_loss = {mode: float("inf") for mode in X18R_MODES}
    final_answer_loss = {mode: float("nan") for mode in X18R_MODES}
    final_total_loss = {mode: float("nan") for mode in X18R_MODES}
    maximum_grad_norm = {mode: 0.0 for mode in X18R_MODES}
    maximum_post4000_grad_norm = {mode: 0.0 for mode in X18R_MODES}
    maximum_training_dual_price = {mode: 0.0 for mode in X18R_MODES}
    ...
    for step in range(1, args.steps + 1):
        ...
        for mode in X18R_MODES:
            ...
            maximum_grad_norm[mode] = max(maximum_grad_norm[mode], row[mode]["grad_norm"])
    ...
    evaluation = {}
    soft_evaluation = {}
    for num_registers in EVAL_CARDINALITIES:
    ...
    report = {
        ...
        "question": "does detaching iterative dual-price history stabilize seen-cardinality optimization?",
        "seed": args.seed,
        ...
        "evaluation_cardinalities": list(EVAL_CARDINALITIES),
        "unseen_evaluation_permitted": False,
        ...
        "dual_contract": {
            ...
            "fullgrad_forward": "lambda=relu(lambda + (sum_rows(softmax(L-lambda))-1))",
            "detached_forward": "same numerical update; updated lambda detached after every round",
            "final_binding": "softmax(L-lambda8)",
            "forward_values_must_match": True,
            "detached_final_binding_to_logits_gradient": True,
        ...
        "representation_contract": {
            "external_descriptor": "exact legacy variable_descriptor(e,n)",
            "candidate_slots": 8,
    ...
    for values in (
        minimum_answer_loss, minimum_total_loss, final_answer_loss, final_total_loss,
        maximum_grad_norm, maximum_post4000_grad_norm, maximum_training_dual_price,
    ):
